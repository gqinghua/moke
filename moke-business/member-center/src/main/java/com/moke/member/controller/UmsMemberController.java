...
public class UmsMemberController {
    ...
    @ApiOperation("更新会员的状态")
    @PostMapping(value = "/update/showStatus")
    public CommonResult updateMembershowStatus(@RequestParam("ids") Long id,@RequestParam("showStatus") Integer start) {
        try {
            IUmsMemberService.updateMembershowStatus(id,start);
            return new CommonResult().success();
        } catch (Exception e) {
            log.error("更新会员状态：%s", e.getMessage(), e);
            return new CommonResult().failed();
        }
    }
}
